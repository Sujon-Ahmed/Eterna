<?php 
    ...
    if(isset($_GET['id'])){
        $id = $_GET['id'];
        $details = $obj-> get_port_details($id);
        if($details->num_rows > 0){
            while($row = $details->fetch_object()){
                $id = $row->portfolio_id;
                $cat_id = $row->portfolio_cat_id;
...
<div class="container-fluid mb-5">
    ...
    <div class="row">
        ...
        <div class="col-lg-8 m-auto">
            <form action="port_image_update.php" method="POST" id="abs" enctype="multipart/form-data">
                <div class="card position-relative shadow-sm">
                    <div class="card-header py-3">
                        <div class="d-flex justify-content-between">
                            <h6 class="m-0 font-weight-bold text-primary">Edit Portfolio Image</h6>
                    ...
                    <div class="card-body">
                        ...
                        <input type="text" name="port_title" value="<?php echo $port_title; ?>" id="card_title" class="form-control" placeholder="portfolio title" required />
                        <!-- category -->
                            ...
                            <?php
                                if($get_cat->num_rows > 0){
                                    while($row = $get_cat->fetch_object()){
                                        ?>
                                            <option <?php if($cat_id == $row->portfolio_cat_id){echo 'selected';} ?> value="<?php echo $row->portfolio_cat_id; ?>"><?php echo $row->cat_name; ?></option>
                                        <?php
                                    }
                                }
                            ?>                           
                        ...
                        <div class="form-row mt-3"> 
                            <div class="form-group col-md-12">
                                ...
                                <input type="hidden" name="oldfile" value="<?php echo $image; ?>" id="img">
                            </div>
                            <div id="port-test-img" class="form-group col-md-6 mt-3">
                                <img class="img-fluid" src="<?php echo 'uploads/portfolio/'.$image; ?>" alt="">
                            </div>
                        </div>
